...
impl From<&crate::types::BlockEvent> for Option<(SidechainNumber, block_info::event::Event)> {
    fn from(event: &crate::types::BlockEvent) -> Self {
        ...
        match event {
            BlockEvent::Deposit(d) => {
                let (n, deposit) = d.into();
                Some((n, block_info::event::Event::Deposit(Box::new(deposit))))
            }
            BlockEvent::SidechainProposal { .. } => None,
            BlockEvent::WithdrawalBundle(b) => {
                let (n, bundle) = b.into();
                Some((
                    n,
                    block_info::event::Event::WithdrawalBundle(Box::new(bundle)),
                ))
            }
        }
    }
...
impl From<&crate::types::BlockEvent> for Option<(SidechainNumber, block_info::Event)> {
    fn from(event: &crate::types::BlockEvent) -> Self {
        let (n, ev) = Option::<_>::from(event)?;
        Some((n, block_info::Event { event: Some(ev) }))
    }
}
    ...
    E: std::borrow::Borrow<crate::types::BlockEvent>,
{
    fn from(info: &crate::types::SidechainBlockInfo<E>) -> Self {
        ...
        let events = info
            ...
            .iter()
            .filter_map(|ev| {
                let (_, ev) = Option::<_>::from(ev.borrow())?;
                Some(ev)
            })
            .collect();
        Self {
